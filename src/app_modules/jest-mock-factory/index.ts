...
class DynamicBase<T extends object> {
    ...
    private readonly stub = Object.create(null);
    private readonly spyProxy: T;
    private readonly spy = Object.create(null);

    // create a spy before it is directly read/written
    private readonly stubProxyHandler = {
        get: (_target: T, propertyName: keyof T, _receiver) => {
            ...
            if (typeof propertyName !== 'string') {
                console.warn(
                    `${propertyName.toString()} is a "${typeof propertyName}" type property. Jasmine can only spy "string" named properties so getting ${propertyName.toString()} will return undefined`,
                );
                return;
            }
        ...
        set: (_target, propertyName: keyof T, value, _receiver) => {
            if (propertyName === '_spy') {
                throw new Error('Cannot modify _spy. It is part of the MockFactory');
            }

            if (typeof propertyName !== 'string') {
                console.warn(
                    `${propertyName.toString()} is a "${typeof propertyName}" type property. Jasmine can only spy "string" named properties so setting ${propertyName.toString()} will be ignored`,
                );
                return true;
            }
            ...
            if (typeof this.prototype[propertyName] === 'function') {
                throw new TypeError(
                    `Cannot change ${propertyName} function, because MockFactory has already attached a permanent spy to it`,
                );
            }
    ...
    private readonly spyProxyHanlder = {
        get: (_target: T, propertyName: keyof T, _receiver) => {
            if (typeof propertyName !== 'string') {
                throw new TypeError(
                    `${propertyName.toString()} is a "${typeof propertyName}" named property. Jasmine can only spy "string" so only "string" named properties expose the _spy interface`,
                );
            }
        ...
        set: (_target, propertyName: keyof T, _value, _receiver) => {
            throw new Error(
                `Cannot change _spy.${propertyName.toString()}, because it is part of the MockFactory`,
            );
        },
    };

    constructor(private readonly prototype: T) {
        this.stubProxy = new Proxy<Mock<T>>(Object.create(null) as Mock<T>, this.stubProxyHandler);
    ...
    private ensureSpy(propertyName: keyof T & string): void {
        ...
        if (!this.spy[propertyName]) {
            try {
                if (typeof this.prototype[propertyName] !== 'function') {
                    // this could throw error on a getter. hence try...catch...
    ...
    private ensureFunction(propertyName: string) {
        ...
        Object.defineProperty(this.spy[propertyName], '_get', {
            get: () => {
                throw new Error(
                    `can't get ${propertyName}._get because ${propertyName} is a function. You can config function spy via ${propertyName}._func`,
                );
            },
            set: () => {
                throw new Error(
                    `can't set ${propertyName}._get because ${propertyName} is a function. You can config function spy via ${propertyName}._func`,
                );
            },
        });
        ...
        Object.defineProperty(this.spy[propertyName], '_set', {
            get: () => {
                throw new Error(
                    `can't get ${propertyName}._set because ${propertyName} is a function. You can config function spy via ${propertyName}._func`,
                );
            },
            set: () => {
                throw new Error(
                    `can't set ${propertyName}._set because ${propertyName} is a function. You can config function spy via ${propertyName}._func`,
                );
            },
        });
    }
    ...
    private ensureProperty(propertyName: string) {
        ...
        const descriptor = {
            ...
            set: /* istanbul ignore next: Can't reach. spyOnProperty() requires its presence to install spies */ (
                _value,
            ) => {},
            enumerable: true,
        ...
        Object.defineProperty(this.spy[propertyName], '_func', {
            get: () => {
                throw new Error(
                    `can't get ${propertyName}._func because ${propertyName} is a property. You can config getter/setter spies via ${propertyName}._get and ${propertyName}._set`,
                );
            },
            set: () => {
                throw new Error(
                    `can't set ${propertyName}._func because ${propertyName} is a property. You can config getter/setter spies via ${propertyName}._get and ${propertyName}._set`,
                );
            },
        });
    }
